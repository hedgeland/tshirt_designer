...
- `printify.py` — Printify API client
...
- **Session Continuity:** Commit and push after every individual change — each discrete edit gets its own commit and immediate `git push origin master`. Don't batch. Use `WIP: <description>` for incomplete work.
...
- **Streaming uses SSE** (`StreamingResponse` + `text/event-stream`).
